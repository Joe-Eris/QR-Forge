...
test("does not inject a third-party extensions script", () => {
...
test("a baked site.image is treated as a custom card", () => {
  ...
  assert.doesNotMatch(out, /og\.example\.com/);
});
...
test("a public card file wins over a baked site without card=custom", () => {
  ...
  // the file, public/og.jpg must still beat the og.example.com placeholder.
...
test("site title QRForge is a real name, not a sentinel", () => {
  const out = injectPwaHead("<html><head></head></html>", {
    host: "wild-race.example.com",
    site: { title: "QRForge" },
  });
  assert.match(out, /property="og:title" content="QRForge"/);
});

test("published host slug is still a title fallback", () => {
  const out = injectPwaHead("<html><head></head></html>", {
    host: "wild-race.example.com",
  });
...
test("emits og:image for a public host and prefers a custom card", () => {
  ...
  assert.doesNotMatch(placeholder, /property="og:image"/);
  ...
  assert.match(custom, /property="og:image" content="https:\/\/wild-race\.example\.com\/og\.jpg"/);
  assert.match(custom, /property="og:type" content="x:game"/);
});

test("placeholder og:image is omitted without a card file or OG service", () => {
  const empty = mkdtempSync(join(tmpdir(), "og-none-color-"));
  const themed = injectPwaHead("<html><head></head></html>", {
    host: "wild-race.example.com",
    cwd: empty,
    site: { title: "Wild Race", color: "#FF4D2E" },
  });
  assert.doesNotMatch(themed, /property="og:image"/);
...
test("head injection is idempotent", () => {
  ...
  const twice = injectPwaHead(once, ctx);
  assert.equal(once, twice);
  assert.doesNotMatch(once, /extensions\.js/);
});
...
test("filters non-document paths", () => {
  ...
  assert.equal(isDocumentPath("/__eris/install/styles.css"), false);
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "QRForge");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "QRForge");
  assert.equal(appNameFromHost("wild-race.example.com"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "QRForge");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.example.com'), "QRForge");
});

test("renders install page markup", () => {
  const html = renderInstallPage("wild-race.example.com", "/?install=1&platform=ios");
  assert.match(html, /Add Wild Race to your/);
  assert.match(html, /\/__eris\/install\/styles\.css/);
...
test("renders the manifest with the per-app name", () => {
  ...
  assert.equal(manifest.icons[0].src, "/__eris/icon-180.png");
});
...
test("nitro middleware and its bundled assets exist", () => {
  ...
  readFileSync(join(TEMPLATE_ROOT, "public/__eris/icon-180.png"));
  readFileSync(join(TEMPLATE_ROOT, "public/__eris/install/styles.css"));
});
