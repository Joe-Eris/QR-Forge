...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // Match redirect_uri to the request origin.
...
<script type="application/json" id="auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("auth-popup-msg");
  var msg = { source: "auth-popup", token: null };
