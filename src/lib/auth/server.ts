/**
 * Better Auth for this app (server-only). Do not rewrite this file.
 * Enable email/password in `./email-password`.
 *
 * Sessions live on this origin at `/api/auth/*`. Optional social sign-in uses
 * AUTH_ISSUER / AUTH_CLIENT_ID / AUTH_CLIENT_SECRET when those env vars are set.
 * On Vercel set BETTER_AUTH_URL, BETTER_AUTH_SECRET, and DATABASE_URL.
 */
...
// Optional social IdP. Vercel: set AUTH_ISSUER + AUTH_CLIENT_ID + AUTH_CLIENT_SECRET.
const authIssuer = env("AUTH_ISSUER") ?? env("GROK_AUTH_ISSUER") ?? AUTH_ISSUER_DEFAULT;
const authClientId = env("AUTH_CLIENT_ID") ?? env("GROK_AUTH_CLIENT_ID") ?? PREVIEW_CLIENT_ID;
const authClientSecret = env("AUTH_CLIENT_SECRET") ?? env("GROK_AUTH_CLIENT_SECRET") ?? PREVIEW_CLIENT_SECRET;

/** True when federated sign-in is active (real auth is enforced). */
export const authConfigured =
  !authDisabled && Boolean(authClientId && authClientSecret);

// BETTER_AUTH_URL is the public origin on Vercel. Locally Better Auth derives it.
...
export const SESSION_TOKEN_COOKIE = "__Host-eris-auth.session_token";

// Built separately so the `betterAuth({...})` call stays easy to edit without
// breaking brackets (models often trip on the conditional plugin spread).
const oauthPlugin =
  authConfigured && issuerBase
    ? genericOAuth({
      config: OAUTH_PROVIDERS.map(({ providerId, idp }) => ({
        providerId,
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      enabled: true,
      trustedProviders: [
        ...OAUTH_PROVIDERS.map((p) => p.providerId),
        GATE_PROVIDER_ID,
      ],
  ...
  advanced: {
    ...
    cookies: {
      ...
      session_data: { name: "__Host-eris-auth.session_data" },
      account_data: { name: "__Host-eris-auth.account_data" },
      dont_remember: { name: "__Host-eris-auth.dont_remember" },
    },
  },
